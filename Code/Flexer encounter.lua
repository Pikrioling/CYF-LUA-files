...
encountertext = "A menacing of Flexers stand in your way!" -- Modify as necessary. It will only be read out in the action select screen.
...
function EncounterStarting()
    ...
    Inventory.AddCustomItem("StaticCandy", "Heals target for 100 HP.", 0, "Player")
    Inventory.AddCustomItem("TVremote", "Allows you to control stuff.", 1, "Enemy")
    Inventory.AddCustomItem("ProteinShake", "Heals 60 HP and boosts attack.", 0, "Player")
    Inventory.AddCustomItem("Nut", "dealer.", 0, "AllPlayer")
    Inventory.AddCustomItem("Tomato", "Throwable vegetable.\nHits hard.", 0, "Enemy")
    Inventory.AddCustomItem("Manual", "Very Useful and Cute", 1, "AllEnemy")

    Inventory.SetInventory({ "StaticCandy", "TVremote", "Manual", "ProteinShake", "Nut", "Tomato", "ProteinShake" })
end
...
function DefenseEnding()
    ...
    DEBUG("yolo")
	enemies[1][3] = math.random(0, 3)
	--enemies[2][4] = math.random(0, 3)
	--enemies[3][4] = math.random(0, 3)]
	enemies[1][4] = true
	--enemies[2][5] = true
	--enemies[3][5] = true
...
function HandleItem(user, targets, itemID, itemData)
    if itemID == "StaticCandy" then
        BattleDialog({"You used the StaticCandy![w:10]\nIt's sparkly."})
		targets[1].Heal(100)
		HandleItem()
    elseif itemID == "TVremote" then
        BattleDialog({"You used the remote on " .. targets[1].name ..".[w:10]\nIt didn't do anything..."})
		HandleItem()
    elseif itemID == "ProteinShake" then
        targets[1].Heal(60)
        BattleDialog({ targets[1].name .. " drank the ProteinShake.[w:10]\nThey regained 60 HP and had their ATK increased!" })
		HandleItem()
    elseif itemID == "Nut" then
        for i = 1, #targets do
            targets[i].Heal(60)
        end
        BattleDialog({"The team splits the Nut apart and eat a part each.[w:10]\nEveryone regained 60 HP!"})
		HandleItem()
    elseif itemID == "Tomato" then
        targets[1].Hurt(100, user)
        BattleDialog({ user.name .. " throws the Tomato at " .. targets[1].name .. ".[w:10]\nIt collides![w:10] " .. targets[1].name .. " loses 100 HP!"})
		HandleItem()
